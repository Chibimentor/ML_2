...
for i = 1:length(sizes)
  for j = 1:num_trials
    ...
    results_bnb(i,j) = mean(hw2_test_bnb(bnb_model,testdata) ~= testlabels);
    op1_model = hw2_train_perc(data(I,:),labels(I),1);
    results_op1(i,j) = mean(hw2_test_perc(op1_model,testdata) ~= testlabels);
    op5_model = hw2_train_perc(data(I,:),labels(I),5);
    results_op5(i,j) = mean(hw2_test_perc(op5_model,testdata) ~= testlabels);
    ap1_model = hw2_train_avgperc(data(I,:),labels(I),1);
    results_ap1(i,j) = mean(hw2_test_perc(ap1_model,testdata) ~= testlabels);
    ap5_model = hw2_train_avgperc(data(I,:),labels(I),5);
    results_ap5(i,j) = mean(hw2_test_perc(ap5_model,testdata) ~= testlabels);
  end
end
figure(1);
hold on;
